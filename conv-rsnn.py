...
dim = torch.Size([lif_layer.n, lif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

lif_lif_conn = Connection(lif_layer,lif_layer, w=w)

dim = torch.Size([out_layer.n, lif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9) #0.95
w[w != 0]=1

o_lif_conn = Connection(
    out_layer,
    lif_layer,
    w=w,  #, w=torch.ones(dim)
    wmin=0,  # minimum weight value
    wmax=1,  # maximum weight value
    update_rule=MSTDPET,  # learning rule
    nu=1e-1,  # learning rate
    norm=0.5 * lif_layer.n,  # normalization)
)
...
dim = torch.Size([alif_layer.n, alif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

alif_alif_conn = Connection(alif_layer,alif_layer, w=w)

dim = torch.Size([out_layer.n, alif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9) #0.95
w[w != 0]=1

o_alif_conn = Connection(
    out_layer,
    alif_layer, 
    w=w,  #, w=torch.ones(dim)
    wmin=0,  # minimum weight value
    ...
    update_rule=MSTDPET,  # learning rule
    nu=1e-1,  # learning rate
    norm=0.5 * alif_layer.n,  # normalization)#, w=torch.ones(dim)
)

inhibitory_layer = LIFNodes(
    n=20, #100 
    #shape=(1, 1, n_filters),
    shape=(20, 1),
    traces=True,
...
dim = torch.Size([inhibitory_layer.n, inhibitory_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

inhibitory_inhibitory_conn = Connection(inhibitory_layer,inhibitory_layer, w=w)

dim = torch.Size([out_layer.n, inhibitory_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9) #0.95
w[w != 0]=1

o_inhibitory_conn = Connection(
    out_layer,
    inhibitory_layer, 
    w=w,  #, w=torch.ones(dim)
    wmin=0,  # minimum weight value
    ...
    update_rule=MSTDPET,  # learning rule
    nu=1e-1,  # learning rate
    norm=0.5 * inhibitory_layer.n,  # normalization)#, w=torch.ones(dim)
)


dim = torch.Size([lif_layer.n, alif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

lif_alif_conn = Connection(lif_layer,alif_layer, w=w)

dim = torch.Size([alif_layer.n, lif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

alif_lif_conn = Connection(alif_layer,lif_layer, w=w)

dim = torch.Size([lif_layer.n, inhibitory_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

lif_inhibitory_conn = Connection(lif_layer,inhibitory_layer, w=w)

dim = torch.Size([inhibitory_layer.n, lif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=-1

inhibitory_lif_conn = Connection(inhibitory_layer,lif_layer, w=w)

dim = torch.Size([alif_layer.n, inhibitory_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=1

alif_inhibitory_conn = Connection(alif_layer,inhibitory_layer, w=w)

dim = torch.Size([inhibitory_layer.n, alif_layer.n])
w = torch.nn.init.sparse_(torch.Tensor(dim), sparsity=0.9)
w[w != 0]=-1

inhibitory_alif_conn = Connection(inhibitory_layer,alif_layer, w=w)
...
alif_pred_conn = Connection(
    ...
    update_rule=MSTDPET,  # learning rule
    nu=1e-1,  # learning rate
    norm=0.5 * alif_layer.n,  # normalization
)

inhibitory_pred_conn = Connection(
    inhibitory_layer,
    pred_layer, 
    w = torch.multiply((0.05 + torch.randn(inhibitory_layer.n, pred_layer.n)), torch.empty(inhibitory_layer.n, pred_layer.n).random_(2)),
    wmin=0,  # minimum weight value
    wmax=1,  # maximum weight value
    update_rule=MSTDPET,  # learning rule
    nu=1e-1,  # learning rate
    norm=0.5 * inhibitory_layer.n,  # normalization
)
...
network.add_layer(inhibitory_layer, name="I")
network.add_layer(pred_layer, name="P")

network.add_connection(out_conn, source="Y", target="O")
network.add_connection(conv_conn, source="X", target="Y")
network.add_connection(recurrent_conn, source="Y", target="Y")
network.add_connection(o_lif_conn, source="O", target="E") 
network.add_connection(o_alif_conn, source="O", target="A") 
network.add_connection(o_inhibitory_conn, source="O", target="I") 
network.add_connection(lif_lif_conn, source="E", target="E")
network.add_connection(alif_alif_conn, source="A", target="A")
network.add_connection(inhibitory_inhibitory_conn, source="I", target="I")
network.add_connection(lif_alif_conn, source="E", target="A")
network.add_connection(alif_lif_conn, source="A", target="E")
network.add_connection(lif_inhibitory_conn, source="E", target="I")
network.add_connection(inhibitory_lif_conn, source="I", target="E")
network.add_connection(alif_inhibitory_conn, source="A", target="I")
network.add_connection(inhibitory_alif_conn, source="I", target="A")
network.add_connection(lif_pred_conn, source="E", target="P")
network.add_connection(alif_pred_conn, source="A", target="P")
network.add_connection(inhibitory_pred_conn, source="I", target="P")
...
for epoch in range(n_epochs):
    ...
    for step, batch in enumerate(tqdm(train_dataloader)):
        ...
        if (label.item() in c_num):
            ...
            # rewarding policy for pred_layer.n=100
            ...
            if plot and batch_size == 1:
                ...
                _spikes = {
                    ...
                    "O": spikes["O"].get("s").view(time, -1) ,
                    "E": spikes["E"].get("s").view(time, -1),
                    "A": spikes["A"].get("s").view(time, -1),
                    "I": spikes["I"].get("s").view(time, -1),
